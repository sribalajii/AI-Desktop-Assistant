...
# NEW: Context memory for better conversations
last_user_input = ""
last_response = ""
pending_action = ""  # For remembering what user asked for
...
def enhanced_local_intent(text):
    """Enhanced command processing with phone features and CONTEXT MEMORY"""
    ...
    if pending_action == "weather" and any(word in text for word in ["yes", "please", "sure", "ok", "okay"]):
        return open_weather_website()

    # Creator questions
    if any(phrase in text for phrase in ["who created", "who made", "your creator", "who are you"]):
    ...
    # FIXED: Voice mode control - Better detection
    elif any(phrase in text for phrase in ["voice mode off", "disable voice", "turn off voice"]):
        VOICE_MODE = False
        response = "Voice mode disabled. Using text mode now."
    elif any(phrase in text for phrase in
             ["voice mode on", "enable voice", "turn on voice", "vioce mode on"]):  # Fixed typo too
        if initialize_voice_mode():
            response = "Voice mode activated! You can now speak to me."
        else:
    ...
    # FIXED: Date and Time together
    elif any(phrase in text for phrase in ["date and time", "time and date"]):
    ...
    # System commands
        ...
        return "EXIT_NOW"
    elif any(k in text for k in ("time", "clock")) and "date" not in text:
        response = tell_time()
    elif any(k in text for k in ("date", "today")) and "time" not in text:
        response = tell_date()
    elif any(k in text for k in ("battery", "power")):
        response = battery_status()

    # FIXED: Weather with context memory
    elif any(k in text for k in ("weather",)):
        pending_action = "weather"  # Remember what user asked for
        response = "I cannot check live weather, but I can open a weather website for you. Should I open it?"

    # Open apps - FIXED: Better edge detection
    ...
    elif text == "edge":  # Direct edge command
        try:
            subprocess.Popen(["msedge.exe"])
...
def process_text_input(text):
    """Process with fixed memory checking and CONTEXT AWARENESS"""
    ...
    # Local commands first (with context)
    ...
    # Ollama for longer queries - DISABLED for faster response
    ...
    fallback_response = "I'm not sure about that. Try: time, date, battery, weather, open [app], call [contact], or sms to [contact]."
...
if __name__ == "__main__":
    ...
    print("🧠 Fixed memory management + Context memory")
    print("=" * 55)
